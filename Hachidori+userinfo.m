...
- (void)savekitsuinfo {
    ...
    NSError *error;
    id responseObject = [self.syncmanager syncGET:@"https://kitsu.app/api/edge/users?filter[self]=true&fields[users]=name,slug,avatar,ratingSystem" parameters:@{} headers:@{} task:NULL error:&error];
    if (!error) {
