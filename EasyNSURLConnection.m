...
-(instancetype)init{
    ...
    useragent =[NSString stringWithFormat:@"%@ %@ (Macintosh; Mac OS X %@; %@)", [NSBundle mainBundle].infoDictionary[@"CFBundleName"],[NSBundle mainBundle].infoDictionary[@"CFBundleShortVersionString"], [NSDictionary dictionaryWithContentsOfFile:@"/System/Library/CoreServices/SystemVersion.plist"][@"ProductVersion"], [NSLocale currentLocale].localeIdentifier];
    return [super init];
}
...
-(void)startoAuthRequest{
    ...
    NXOAuth2Request *sRequest = [[NXOAuth2Request alloc] initWithResource:URL
                                                                   method:@"GET"
                                                               parameters:nil];
    sRequest.account = [self getFirstAccount];
...
-(void)startFormRequest{
    ...
    [lock lock];
    //Set Post Data
    request.HTTPBody = [self encodeArraywithDictionaries:formdata];
    // Set Other headers, if any
    if (headers != nil) {
        for (NSDictionary *d in headers ) {
            //Set any headers
...
- (NSData*)encodeArraywithDictionaries:(NSArray*)array {
    ...
    for (NSDictionary * d in array) {
        NSString *encodedValue = [d[d.allKeys[0]] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        NSString *encodedKey = [d.allKeys[0] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
